...
const quotes = [
  { text: "The grind you don't see today will define the life you admire tomorrow.", tags: ['#DevOps', '#CloudEngineering'] },
  { text: "Automate everything you do more than twice. Your future self will thank you.", tags: ['#Automation', '#DevOps'] },
  { text: "Infrastructure as Code isn't just a practice — it's a mindset shift.", tags: ['#Terraform', '#IaC'] },
  { text: "A pipeline that fails fast is better than one that fails in production.", tags: ['#CI/CD', '#GitHubActions'] },
  { text: "Kubernetes doesn't make things simple. It makes complex things manageable.", tags: ['#Kubernetes', '#Cloud'] },
];

const Blog: React.FC = () => {
  const [current, setCurrent] = useState(0);
  const [liked, setLiked] = useState<boolean[]>(quotes.map(() => false));
  const [likeCounts, setLikeCounts] = useState<number[]>([9800, 4200, 6100, 3300, 7700]);
  ...
  return (
    <div className="min-h-screen bg-white pt-32 pb-24 px-6 md:px-12 max-w-4xl mx-auto">
      ...
      {/* Quote Carousel */}
      <div className="mb-16 p-[2px] rounded-[1.5rem] bg-gradient-to-r from-blue-500 to-blue-400">
